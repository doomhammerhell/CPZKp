...
mod tests {
    ...
    proptest! {
        ...
        fn test_serialization_roundtrip() {
            // Test scalar point
            let point = Point::Scalar(BigUint::from(65256u32));
            let serialized = point.serialize();
            ...
            // Test EC point
            let point = Point::ECPoint(
                BigUint::from(65256u32),
                BigUint::from(8475u32),
            );
            let serialized = point.serialize();
            let deserialized = Point::deserialize(serialized, &Group::EllipticCurve).unwrap();
            assert_eq!(point, deserialized);
        }
